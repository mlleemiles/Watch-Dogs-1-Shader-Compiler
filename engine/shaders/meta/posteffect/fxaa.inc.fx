...
#idef NOMAD_PLATFORM_CURRENTGEN
...
#define NexusFxaaProcessInput(c)	pow(saturate(c), 1.0f / 2.2f)
#define NexusFxaaProcessOutput(c)	pow(saturate(c), 2.2f)
...
float4 FxaaTexLod0(FxaaTex tex, float2 pos) {
    #if FXAA_GLSL_120
        return NexusFxaaProcessInput(texture2DLod(tex, pos.xy, 0.0));
    #endif
    #if FXAA_GLSL_130
        return NexusFxaaProcessInput(textureLod(tex, pos.xy, 0.0));
    #endif
    #if FXAA_HLSL_3
        return NexusFxaaProcessInput(tex2Dlod(tex, float4(pos.xy, 0.0, 0.0))); 
    #endif
    #if FXAA_HLSL_4
        return NexusFxaaProcessInput(tex.tex.SampleLevel(tex.smpl, pos.xy, 0.0));
    #endif
}
/*--------------------------------------------------------------------------*/
float4 FxaaTexGrad(FxaaTex tex, float2 pos, float2 grad) {
    #if FXAA_GLSL_120
        return NexusFxaaProcessInput(texture2DGrad(tex, pos.xy, grad, grad));
    #endif
    #if FXAA_GLSL_130
        return NexusFxaaProcessInput(textureGrad(tex, pos.xy, grad, grad));
    #endif
    #if FXAA_HLSL_3
        return NexusFxaaProcessInput(tex2Dgrad(tex, pos.xy, grad, grad)); 
    #endif
    #if FXAA_HLSL_4
        return NexusFxaaProcessInput(tex.tex.SampleGrad(tex.smpl, pos.xy, grad, grad));
    #endif
}
/*--------------------------------------------------------------------------*/
float4 FxaaTexOff(FxaaTex tex, float2 pos, int2 off, float2 rcpFrame) {
    #if FXAA_GLSL_120
        return NexusFxaaProcessInput(texture2DLodOffset(tex, pos.xy, 0.0, off.xy));
    #endif
    #if FXAA_GLSL_130
        return NexusFxaaProcessInput(textureLodOffset(tex, pos.xy, 0.0, off.xy));
    #endif
    #if FXAA_HLSL_3
        return NexusFxaaProcessInput(tex2Dlod(tex, float4(pos.xy + (off * rcpFrame), 0, 0))); 
    #endif
    #if FXAA_HLSL_4
        return NexusFxaaProcessInput(tex.tex.SampleLevel(tex.smpl, pos.xy, 0.0, off.xy));
    #endif
}
...
float3 FxaaFilterReturn(float3 rgb) {
    ...
    #else
        return NexusFxaaProcessOutput(rgb);
    #endif
}
...
FxaaTex CreateTex(in Texture_2D s, Texture2D t)
{
#if FXAA_HLSL_3
	return s;
#else
...
{
	FxaaTex	tex = CreateTex( AnisotropicSampler, InputTexture ); 
